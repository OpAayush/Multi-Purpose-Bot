  ...
  try {
    ...
    if(cmd.length == 0 || !cmd){
        if(mPrefix.includes(client.user.id)){
            message.reply({embeds: [new Discord.EmbedBuilder().setColor(ee.color).setFooter({ text: ee.footertext, iconURL: ee.footericon}).setTitle(`:thumbsup: **My Prefix here, is __\`${prefix}\`__**`)]})
        }
        return;
    }
    ...
    if (command) {
        ...
        try {
          ...
          command.messageRun(client, message, args, args.join(" ").split("++").filter(Boolean), message.member, args.join(" "), prefix);
        } catch (error) {
