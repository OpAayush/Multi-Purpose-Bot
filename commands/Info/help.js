      ...
      if (args && args.length > 0) {
        ...
        if (!cmd) {
          ...
          return interaction.reply({
            ephemeral: true,
            embeds: [new EmbedBuilder().setColor(ee.wrongcolor).setDescription(errorMessage)],
          });
        }
        ...
        embed.addFields({
          name: "**Cooldown**",
          value: cmd.cooldown ? `\`${cmd.cooldown} Seconds\`` : `\`${settings.default_cooldown_in_sec} Second\``,
        });
...
- Do NOT type these when using commands!`)}
    ...
    - Type \`${prefix}help <command | module>\` for more info.`
        ...
        const options = client.categories.map(category => ({
          label: category,
          value: category.toLowerCase(),
          description: `Commands count: ${commandsByCategory(category).length}`,
          emoji: client.allEmojis[category.toLowerCase()] || undefined,
        }));
        ...
        const filterMenu = interaction => interaction.customId === 'help_menu_select';
        let collector;
        ...
        collector.on('collect', async (response) => {
          if (response.customId === 'help_menu_select') {
            ...
            const categoryCommands = commandsByCategory(capitalizeFirstLetter(selectedCategory));
            const categoryEmbed = new EmbedBuilder()
              .setDescription(`**__${capitalizeFirstLetter(selectedCategory.toUpperCase())}__ [${categoryCommands.length}]**\n \n - ${categoryCommands.join(", ")}`);
      ...
      return interaction.followUp({
        ...
        embeds: [
          new EmbedBuilder()
            ...
            .setDescription(`\`\`\`${String(error.message || error).substr(0, 2000)}\`\`\``),
        ],
      });
    }
